...
Every event's `regarding` is the object the controller acted on (a `Wavefront`, or — for a pin event's mirrored copy — the `GitRepository`), `related` names the secondary object the action concerns (the `GitRepository` a per-source `Wavefront`-regarding event is about, or vice versa on the mirrored copy), and `action` is a short machine-readable verb (`Pin`, `ShadowPin`, `Hold`, `Release`, `Demote`) distinct from `reason`'s outcome. This matters beyond taxonomy: the events.k8s.io/v1 recorder deduplicates on `{type, action, reason, reportingController, reportingInstance, regarding, related}` — the note is not part of that key — so a per-source event that omitted `related` would collapse onto one Event with every other source sharing the same `regarding`/`reason` in the same pass, discarding all but the first source's message.
...
- **Events:** `PinAdvanced`, `InitialPin`, `HoldDetected`/`HoldReleased`, `ShadowAdmission` — the admission ledger (D14). Each per-source event names its source's `GitRepository` as `related` (§4.2), so several sources acted on in the same pass record as distinct events rather than collapsing into one.
