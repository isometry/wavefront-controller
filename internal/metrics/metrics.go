...
type Instruments struct {
	...
	// unadmitted observed revision. Recomputed wholesale every pass.
	...
	// Recomputed wholesale every pass.
	...
	// sourcev1 FetchFailed condition (§10 force-push detection).
	// Recomputed wholesale every pass.
...
func New(reg prometheus.Registerer) *Instruments {
	return &Instruments{
		...
		PinLagSeconds: register(reg, prometheus.NewGaugeVec(prometheus.GaugeOpts{
			...
			Help: "Age in seconds of a node's currently unadmitted observed revision.",
		...
		BlockedNodes: register(reg, prometheus.NewGaugeVec(prometheus.GaugeOpts{
			...
			Help: "Number of nodes currently blocked, by reason.",
		}, []string{"reason"})),
		...
		PinnedFetchFailures: register(reg, prometheus.NewGauge(prometheus.GaugeOpts{
			Name: "wavefront_pinned_fetch_failures",
			Help: "Number of pinned sources currently reporting a fetch failure.",
		})),
	}
}
